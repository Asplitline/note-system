<template>
  <div class="my-note">
    <Card title="我的笔记">
      <ul class="note-tabs">
        <li v-for="i in POST_STATE" :key="i.type" class="note-tabs-item" :class="{ active: i.type === active }" @click="setActive(i)">
          {{ i.text }}
        </li>
      </ul>
      <div class="note-tabpane">
        <template v-if="notEmpty(filterList)">
          <div v-for="i in filterList" :key="i.id" class="note-brief">
            <div class="brief-box">
              <h3 @click="gotoDetail(i)">
                {{ i.title }} <span v-if="i.type === 1" class="fa fa-jpy">{{ i.price }}</span>
              </h3>
              <p>{{ i.title }}</p>
              <div class="other">
            ...
            <div class="cover-box">
              <img src="http://www.bianbiangou.cn/index/ICON2.png" alt="" @click="gotoDetail(i)" />
            </div>
          </div>
        </template>
        <template v-else>
