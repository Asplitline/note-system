    ...
    <template v-if="notEmpty(list)">
      <div v-for="i in list" :key="i.id" class="item">
        <div class="cover">
          ...
          <img :src="_url(i.coverImage)" alt="" class="img" onerror="this.src='http://www.bianbiangou.cn/index/ICON2.png'" />
        </div>
        <div class="info">
          <div class="title">
            {{ i.title }}
          ...
          <div class="other">
            <div class="date"><i class="fa fa-clock-o" />{{ i.createTime | formatDate }}</div>
            <div class="view"><i class="fa fa-eye" />浏览({{ i.view || 0 }})</div>
            <div class="view"><i class="fa fa-thumbs-o-up" />点赞({{ i.likeNum || 0 }})</div>
            <div class="comment"><i class="fa fa-commenting-o" />评论({{ i.replyNum || 0 }})</div>
          </div>
        </div>
        <div class="btns">
          <button class="btn" @click="handleClick(i)">
            查看笔记
          </button>
        </div>
      </div>
