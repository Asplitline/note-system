<template>
  <div class="asider">
    <myCard title="关于我" icon="fa-user">
      <div class="about">
        <div class="qr-code">
          <img :src="file_url(currentUser.fileUrl)" alt="" onerror="this.src='http://www.bianbiangou.cn/index/ICON2.png'" />
        </div>
        <div class="author">
    ...
    <myCard v-if="notEmpty(toc)" title="目录" icon="fa-book" class="toc-card">
      <ul class="toc">
        <li v-for="i in toc" :key="i.id" class="toc-item">
          <a href="javascript:;" class="link" @click="achor(i)">{{ i.text }}</a>
        </li>
      </ul>
    </myCard>
    <myCard title="标签云" icon="fa-tag">
      <div class="tag">
        <router-link v-for="i in tagList" :key="i.id" :to="{ name: 'pTag', params: { id: i.id } }" class="tag-link">
          {{ i.name }}
        </router-link>
      </div>
    </myCard>
    <myCard title="近期评论" icon="fa-comment">
      <ul class="comment">
        <li v-for="i in commentList" :key="i.id" class="comment-item">
          <img :src="file_url(i.extend1)" alt="" class="img" onerror="this.src='http://www.bianbiangou.cn/index/ICON2.png'" />
    ...
    <myCard title="近期笔记" icon="fa-book">
      <ul class="note-list">
        <li v-for="i in noteList" :key="i.id" class="note-item" @click="gotoArticle(i)"><i class="fa fa-book" />{{ i.title }}</li>
      </ul>
    </myCard>
