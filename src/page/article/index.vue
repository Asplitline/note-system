    ...
    <el-row :gutter="60" class="w">
      <el-col :span="18" class="detail">
        <div class="main">
            ...
            <div class="top-info">
              ...
              <div v-if="notEmpty(currentAuthor)" class="info">
                <div class="author-info">
                  ...
                  <button class="btn" :class="{ focus: isFocus }" @click="focusUser">
                    {{ isFocus ? '已关注' : '关注Ta' }}
                  </button>
                </div>
                <div class="other">
                  <div class="date"><i class="fa fa-clock-o" />{{ currentPost.createTime | formatDate }}</div>
                  <div class="view"><i class="fa fa-eye" />浏览({{ currentPost.view || 0 }})</div>
                  <div class="view"><i class="fa fa-thumbs-o-up" />点赞({{ currentPost.likeNum || 0 }})</div>
                  <div class="comment"><i class="fa fa-commenting-o" />评论({{ currentPost.replyNum || 0 }})</div>
                </div>
              </div>
            </div>
          ...
          <div class="note">
            <p v-if="notEmpty(currentType)">
              所属分类：<span class="category" @click="gotoList(currentType.id)">{{ currentType.name }}</span>
            </p>
            <!-- <p>本文标签：<span class="tag" v-for="i in 4">javascript</span></p> -->
          </div>
        </div>
        <div class="footer">
          <div v-if="false" class="goto">
            <div class="pre">
              <p><i class="fa fa-angle-left" />上一篇</p>
              <p>阿斯顿萨达</p>
            </div>
            <div class="next">
              <p>下一篇<i class="fa fa-angle-right" /></p>
              <p>阿斯顿萨达</p>
            </div>
          </div>
          <myCard :shadow="false" title="热门推荐" class="hots" icon="fa-fire">
            <ul class="hot-list">
              <li v-for="i in randNotes" :key="i.id" class="hot-item">
                <div class="cover">
                  <img :src="file_url(i.coverImage)" alt="" onerror="this.src='http://www.bianbiangou.cn/index/ICON2.png'" />
                </div>
                <div class="info">
                  <div class="title" @click="gotoDetail(i)">
                    {{ i.title }}
                  </div>
                  <div class="other">
          ...
          <myCard v-if="false" :shadow="false" title="相关笔记" class="related" icon="fa-bookmark-o">
            <ul class="related-list">
              <li v-for="i in 4" :key="i" class="related-item">
                <i class="fa fa-book" />
              </li>
            </ul>
          </myCard>
          <myCard :shadow="false" title="评论" class="comment" icon="fa-commenting-o">
            ...
            <div class="comment-list">
              <h3 class="comment-title">全部评论 ({{ notEmpty(commentList) ? commentList.length : 0 }})</h3>
              <template v-if="notEmpty(commentList)">
                <div v-for="i in commentList" :key="i.id" class="comment-item">
                  <div class="header">
                    <img :src="file_url(i.author.fileUrl)" alt="" onerror="this.src='http://www.bianbiangou.cn/index/ICON2.png'" />
                    <div class="info">
                      <span class="author">{{ i.author.userName }}</span
                      ><span class="date fa fa-clock-o">{{ i.createTime | formatDate }}</span>
                    </div>
                  </div>
                  <div class="content">
                    <div class="reply">
                      ...
                      <ul class="reply-list">
                        <li v-for="reply in getReplyById(i.id)" :key="reply.id" class="reply-item">
                          <div class="reply-header">
                            ...
                            <span class="date fa fa-clock-o">{{ reply.createTime | formatDate }}</span>
                          </div>
                  ...
                  <div class="footer">
                    <span class="fa fa-thumbs-o-up" @click="likeComment(i)">赞({{ i.extend2 || 0 }})</span>
                    <span class="fa fa-reply" @click="setCurrent(i)">回复</span>
                  </div>
                  <commentBox v-if="current === i" v-model="reply" :is-reply="true" @handle="addReply(i)" />
                </div>
              </template>
...
export default {
  ...
  methods: {
    ...
    genarateTOC() {
      ...
      const h = [...this.$refs.html.childNodes].filter((i) => headList.includes(i.localName))
    ...
    async fetchStarUserList() {
      ...
      this.isFocus = payload.findIndex((i) => i.id === this.currentPost.userId) !== -1
    },
    async focusUser() {
      if (this.isFocus) {
        const { success, message } = await deleteOne(API.FOCUSON, {
  ...
  computed: {
    ...mapGetters(['currentPost', 'userlist', 'currentUser', 'types', 'userId', `user/${type.GET_USER}`]),
    currentAuthor() {
