  ...
  <header class="header">
    <div class="w">
            ...
            <template v-if="notEmpty(i.children)">
              <el-dropdown trigger="click">
                <span class="el-dropdown-link">
                  <i class="fa" :class="i.icon" />{{ i.name }}<i class="el-icon-arrow-down el-icon--right" />
                </span>
                <el-dropdown-menu slot="dropdown">
                  <el-dropdown-item v-for="j in i.children" :key="j.id" class="dropdown-item">
      ...
      <div class="search-box">
        <el-input v-model="keyword" placeholder="搜索文章...">
          <i slot="suffix" class="fa fa-search" :class="{ disabled: !isSearch }" @click="search" />
        </el-input>
      </div>
      <div class="login-box">
        <div class="edit-post">
          <button class="edit-btn" @click="goto"><i class="fa fa-edit" />写文章</button>
        </div>
        <el-dropdown trigger="click" class="login-dropdown" @command="handleCommand">
          ...
          <el-dropdown-menu slot="dropdown">
            <el-dropdown-item command="info"> <i class="fa fa-sign-out" />个人中心 </el-dropdown-item>
            <el-dropdown-item v-if="isAuth" command="home"> <i class="fa fa-home" />后台系统 </el-dropdown-item>
            <el-dropdown-item command="logout"> <i class="fa fa-user-o" />退出登录 </el-dropdown-item>
          </el-dropdown-menu>
        </el-dropdown>
      </div>
...
export default {
  ...
  computed: {
    ...
    myNav() {
      const nav = this.$router.options.routes.filter((i) => i.hasPage)[0].children
      return nav.filter((i) => i.show)
    },
    fliterList() {
      const parent = this.list.filter((i) => i.pid === null && i.available === 1)
      const all = parent.map((i) => {
        const children = this.list.filter((j) => j.pid === i.id && i.available === 1)
        return {
          ...i,
