<template>
  <el-card class="reply">
    <TableHeader v-model="query.keyword" :show-left="false" @search="getList" @clear="getList" />
    <el-table v-loading="listLoading" :data="list" element-loading-text="Loading" border style="width: 100%" fit highlight-current-row>
      <el-table-column align="center" label="笔记标题" min-width="95">
        <template v-slot="{ row }">
          <div v-if="notEmpty(row.note)" class="t-ellipsis">
            {{ row.note.title }}
          </div>
        </template>
      </el-table-column>
      <el-table-column align="center" label="笔记作者" min-width="95">
        <template v-slot="{ row }">
          <div v-if="notEmpty(row.author)" class="t-ellipsis">
            {{ row.author.userName }}
          </div>
        </template>
      </el-table-column>
      <el-table-column align="center" label="评论用户" min-width="95">
        <template v-slot="{ row }">
          <div v-if="notEmpty(row.commentAuthor)" class="t-ellipsis">
            {{ row.commentAuthor.userName }}
          </div>
        </template>
      </el-table-column>
      <el-table-column align="center" label="回复用户" min-width="95">
        <template v-slot="{ row }">
          <div v-if="notEmpty(row.replyAuthor)" class="t-ellipsis">
