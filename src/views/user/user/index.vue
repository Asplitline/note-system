<template>
  <el-card class="tag">
    ...
    <el-table v-loading="listLoading" :data="list" element-loading-text="Loading" border style="width: 100%" fit highlight-current-row>
      <el-table-column align="center" label="头像" min-width="95">
        <template v-slot="{ row }">
          <el-avatar v-if="row.fileUrl" shape="square" :size="80" :src="_url(row.fileUrl)" />
      ...
      <el-table-column align="center" class-name="t-btns" label="操作" min-width="95">
        <template v-slot="{ row }">
          <!-- {{ scope.$index }} -->
          <el-button class="t-btn" type="success" icon="el-icon-edit" @click="editById(row)" />
          <el-button slot="reference" class="t-btn" type="danger" icon="el-icon-delete" @click="deleteById(row.id)" />
          <el-button
            slot="reference"
            class="t-btn"
            type="primary"
...
"
    ...
    >
      <el-form ref="userForm" :model="userForm" :hide-required-asterisk="true" status-icon :rules="rules" label-width="80px">
        <el-form-item v-if="isEdit" label="用户头像" prop="file_url">
      ...
      <div slot="footer" class="dialog-footer t-btns">
        ...
        <el-button type="success" class="t-btn" @click="submitForm('userForm')">
          {{ isEdit ? '修 改' : '添 加' }}
        </el-button>
      </div>
    </el-dialog>
...
<script>
import { API, insertOne, deleteOne, getUser, changePassword, editUser } from '@/api'
